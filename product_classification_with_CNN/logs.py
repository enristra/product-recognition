...
def save_multitask_log(model, history, test_results, config_dict, logs_dir):
    logs_dir = Path(logs_dir)
    logs_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
    log_path = logs_dir / f"{model.name}_{timestamp}.txt"
    ...
    tasks = {
        "Fine Class (f)": {
            "train_acc": hist.get("f_accuracy", []),
            "val_acc": hist.get("val_f_accuracy", []),
            "train_loss": hist.get("f_loss", []),
            "val_loss": hist.get("val_f_loss", []),
            "test_loss": test_results['f_loss'],    # loss fine
            "test_acc": test_results['f_accuracy'],     # accuracy fine
        },
        "Super Class (s)": {
            "train_acc": hist.get("s_accuracy", []),
            "val_acc": hist.get("val_s_accuracy", []),
            "train_loss": hist.get("s_loss", []),
            "val_loss": hist.get("val_s_loss", []),
            "test_loss": test_results['s_loss'],    # loss super
            "test_acc": test_results['s_accuracy'],     # accuracy super
        },
    }
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        f.write(f"EXPERIMENT REPORT {model.name}_{timestamp}\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write(f"Total Loss (test): {test_results['loss']:.4f}\n\n")

        for task_name, m in tasks.items():
            f.write(f"{task_name}\n")
            f.write("-" * 60 + "\n")
            best_epoch = max(range(len(m["val_acc"])), key=lambda i: m["val_acc"][i]) + 1 if m["val_acc"] else None
            f.write(f"Best epoch (val_acc): {best_epoch}\n")
            f.write(f"Best train acc: {max(m['train_acc']):.4f}\n")
            f.write(f"Best val acc:   {max(m['val_acc']):.4f}\n")
            f.write(f"Test acc:       {m['test_acc']*100:.2f}%\n")
            f.write(f"Test loss:      {m['test_loss']:.4f}\n\n")
